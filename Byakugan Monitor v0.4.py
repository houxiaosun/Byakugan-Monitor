...
# 创建一个主窗口
window = QMainWindow()
window.setWindowTitle("Byakugan Monitor v0.4") # 主窗口名
...
table.setHorizontalHeaderLabels(["时间", "CPU使用率", "内存使用率"])
...
def update_all():
    ...
    log_data(cpu_usage, mem_usage)  # 新增日志记录

timer.timeout.connect(update_all)#直接关连计时器与总函数

def log_data(cpu_usage, mem_usage):
